<template>
  <teleport to="body">
    <!-- <div v-if="isOpen" class="fixed inset-0 z-[100000] overflow-y-auto" @click.self="close"> -->
      <div v-if="isOpen" class="fixed inset-0 z-[100000] overflow-y-auto" @click.self="close">
      <!-- Backdrop -->
      <div class="fixed inset-0 bg-black/50 backdrop-blur-sm transition-opacity" @click="close"></div>

      <!-- Modal Container -->
      <div class="flex min-h-full items-center justify-center p-4 lg:pl-[90px]">
        <div
          ...
          :class="{
            ...
            'max-w-4xl': size === 'xl',
            'max-w-6xl': size === '2xl',
            'w-full lg:w-8/12 lg:max-w-none': size === 'full',
          }">
          <slot name="body"></slot>
        </div>
      </div>
