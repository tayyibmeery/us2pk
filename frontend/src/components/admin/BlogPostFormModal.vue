<template>
  <FormModal :isOpen="isOpen" :title="formData.id ? 'Edit Blog Post' : 'Add Blog Post'"
    :subtitle="formData.id ? 'Update the blog post details below.' : 'Fill in the details to add a new blog post.'"
    ...
    <template #fields>
      <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
        ...
        <div class="sm:col-span-2">
          ...
          <input v-model="formData.title" type="text" placeholder="e.g. Top 10 Shipping Tips"
        ...
        <div>
          ...
          <input v-model="formData.slug" type="text" placeholder="e.g. top-10-shipping-tips"
            class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
        ...
        <div>
          ...
          <input v-model="formData.author" type="text" placeholder="e.g. John Doe"
            class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
        </div>
        ...
        <div class="sm:col-span-2">
          ...
          <input v-model="formData.section_title" type="text" placeholder="e.g. Latest Articles"
            class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
        </div>
        ...
        <div class="sm:col-span-2">
          ...
          <textarea v-model="formData.excerpt" rows="2" placeholder="Brief summary of the post"
            class="dark:bg-dark-900 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30"></textarea>
        </div>
        ...
        <div class="sm:col-span-2">
          ...
          <textarea v-model="formData.content" rows="6" placeholder="Full blog post content"
            class="dark:bg-dark-900 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30"
            required></textarea>
        </div>
...
import type { BlogPost } from '@/stores/blogPostStore'
...
const publishedAt = computed({
  get: () => formData.value.published_at ? new Date(formData.value.published_at).toISOString().split('T')[0] : '',
  set: (value: string) => {
    formData.value.published_at = value ? new Date(value).toISOString() : null
  }
})
...
const save = () => {
  ...
  const fields = ['title', 'slug', 'excerpt', 'content', 'section_title', 'section_content', 'author']
  ...
  if (formData.value.published_at) {
