<template>
  <FormModal :isOpen="isOpen" :title="formData.id ? 'Edit Testimonial' : 'Add Testimonial'"
    :subtitle="formData.id ? 'Update the testimonial details below.' : 'Fill in the details to add a new testimonial.'"
    ...
    <template #fields>
      <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
        ...
        <div>
          ...
          <input v-model="formData.title" type="text" placeholder="e.g. CEO, Company"
            class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
        </div>
        ...
        <div class="sm:col-span-2">
          ...
          <textarea v-model="formData.content" rows="3" placeholder="Testimonial text"
...
const formData = ref<Partial<Testimonial>>({
  name: '',
  title: '',
  content: '',
  image: null,
...
const save = () => {
  ...
  if (formData.value.rating !== undefined && formData.value.rating !== null) {
    form.append('rating', String(formData.value.rating))
  }
