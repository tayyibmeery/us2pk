<template>
  <FormModal :isOpen="isOpen" :title="formData.id ? 'Edit About Section' : 'Add About Section'"
    :subtitle="formData.id ? 'Update the about section details below.' : 'Fill in the details to add a new about section.'"
    ...
    <template #fields>
      <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
        ...
        <div class="sm:col-span-2">
          ...
          <input v-model="formData.title" type="text" placeholder="e.g. About US2PK"
        ...
        <div class="sm:col-span-2">
          ...
          <input v-model="formData.subtitle" type="text" placeholder="e.g. About Us"
            class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
        </div>
        ...
        <div class="sm:col-span-2">
          ...
          <textarea v-model="formData.content" rows="4" placeholder="Main content about the company"
        ...
        <!-- Features - Use a simple text input instead of JSON textarea -->
        <div class="sm:col-span-2">
          <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
            Features (JSON)
          </label>
          <textarea v-model="featuresJson" rows="6"
            placeholder='[{"icon": "shield", "title": "Secure", "description": "Advanced security"}]'
          ...
          <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Enter valid JSON array of features</p>
        </div>
...
import type { AboutSection } from '@/stores/aboutSectionStore'
...
const formData = ref<Partial<AboutSection>>({
  title: '',
  subtitle: '',
  content: '',
  image: null,
...
// Watch for changes in featuresJson and parse it
const parseFeatures = () => {
  try {
...
// Update featuresJson when initialData changes
watch(
  () => props.initialData,
  (newVal) => {
    if (newVal) {
      formData.value = { ...newVal, status: !!newVal.status }
      if (newVal.features && Array.isArray(newVal.features) && newVal.features.length > 0) {
        featuresJson.value = JSON.stringify(newVal.features, null, 2)
      } else {
        featuresJson.value = '[]'
      }
    } else {
...
const save = () => {
  ...
  // Parse features and add as JSON string
  const parsed = parseFeatures()
  if (parsed && formData.value.features && Array.isArray(formData.value.features) && formData.value.features.length > 0) {
    form.append('features', JSON.stringify(formData.value.features))
  } else if (formData.value.features && Array.isArray(formData.value.features) && formData.value.features.length > 0) {
    form.append('features', JSON.stringify(formData.value.features))
  }
