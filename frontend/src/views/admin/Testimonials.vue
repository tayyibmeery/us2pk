<template>
  <div class="space-y-6">
    ...
    <DataTable :store="testimonialStore" :columns="columns" title="Testimonials" addButtonLabel="Add Testimonial"
      :modalComponent="TestimonialFormModal">
      ...
      <!-- Custom rating column rendering -->
      <template #cell-rating="{ item }">
        <div class="flex items-center gap-0.5">
          <span v-for="n in 5" :key="n" class="text-sm"
            :class="n <= item.rating ? 'text-yellow-400' : 'text-gray-300 dark:text-gray-600'">
            ★
          </span>
        </div>
      </template>
...
import TestimonialFormModal from '@/components/admin/TestimonialFormModal.vue'
