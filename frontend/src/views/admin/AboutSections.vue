<template>
  <div class="space-y-6">
    ...
    <DataTable :store="aboutSectionStore" :columns="columns" title="About Sections" addButtonLabel="Add About Section"
      :modalComponent="AboutSectionFormModal">
...
import AboutSectionFormModal from '@/components/admin/AboutSectionFormModal.vue'
import { useAboutSectionStore } from '@/stores/aboutSectionStore'
...
const getImageUrl = (imagePath: string | null | undefined): string => {
  ...
  if (imagePath.startsWith('storage/')) return `${baseUrl}/${imagePath}`
  if (imagePath.startsWith('about/')) return `${baseUrl}/storage/${imagePath}`
...
onMounted(() => {
  aboutSectionStore.fetchItems(1)
})
</script>
