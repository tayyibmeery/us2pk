<template>
  <div class="space-y-6">
    <PageBreadcrumb :pageTitle="'Why Us Sections'" />
    <DataTable :store="whyUsSectionStore" :columns="columns" title="Why Us Sections" addButtonLabel="Add Why Us Section"
      :modalComponent="WhyUsSectionFormModal">
      ...
      <!-- Custom features count column rendering -->
...
import WhyUsSectionFormModal from '@/components/admin/WhyUsSectionFormModal.vue'
import { useWhyUsSectionStore } from '@/stores/whyUsSectionStore'
import type { ColumnDefinition } from '@/types/table'

const whyUsSectionStore = useWhyUsSectionStore()
...
const getImageUrl = (imagePath: string | null | undefined): string => {
  ...
  if (imagePath.startsWith('storage/')) return `${baseUrl}/${imagePath}`
  if (imagePath.startsWith('whyus/')) return `${baseUrl}/storage/${imagePath}`
...
onMounted(() => {
  whyUsSectionStore.fetchItems(1)
})
</script>
