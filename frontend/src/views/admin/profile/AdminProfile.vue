<!-- frontend/src/views/admin/profile/AdminProfile.vue -->
<template>
  <div class="space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-4">
      <div>
        <h1 class="text-2xl font-bold text-gray-800 dark:text-white">Admin Profile</h1>
        <p class="text-sm text-gray-500 dark:text-gray-400">View and manage your administrator profile</p>
      </div>
      <div class="flex gap-3">
        ...
        <button @click="openEditModal"
          class="inline-flex items-center gap-2 px-4 py-2.5 text-sm font-medium text-white bg-purple-500 rounded-lg hover:bg-purple-600 transition shadow-sm hover:shadow-md">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <div v-if="loading" class="flex justify-center py-16">
      <div
        class="w-8 h-8 border-4 border-gray-200 dark:border-gray-700 border-t-purple-500 rounded-full animate-spin" />
    </div>

    <div v-else
      class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
      <div class="h-20 bg-gradient-to-r from-purple-500 to-purple-600 dark:from-purple-600 dark:to-purple-700" />
      ...
      <div class="px-6 pb-6">
        <div class="flex flex-col sm:flex-row sm:items-end gap-4 -mt-12">
          <div class="relative group shrink-0">
            <div
              ...
              <img :src="adminAvatar" alt="Admin" class="w-full h-full object-cover" @error="handleImageError" />
            </div>
            <label for="avatar-upload-admin"
              class="absolute bottom-0 right-0 flex items-center justify-center w-8 h-8 bg-purple-500 rounded-full cursor-pointer hover:bg-purple-600 transition shadow-sm border-2 border-white dark:border-gray-800"
              title="Change photo">
            ...
            <input id="avatar-upload-admin" type="file" accept="image/*" class="hidden" :disabled="uploading"
        ...
        <div class="flex flex-wrap items-center gap-2 mt-5">
          <span
            class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-purple-50 text-purple-700 dark:bg-purple-900/20 dark:text-purple-400">
        ...
        <div class="mt-8">
          <h3 class="text-sm font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-4">Administrator
            Information</h3>
          <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <InfoField label="Full Name" :value="user?.name" />
            <InfoField label="Email Address" :value="user?.email" locked />
            <InfoField label="Phone Number" :value="user?.phone" />
            <InfoField label="PCode" :value="user?.pcode" locked />
            <InfoField label="City" :value="user?.city?.city_name" locked />
            <InfoField label="Tax ID" :value="user?.tax_id" />
    ...
    <EditProfileModal :isOpen="showEditModal" :isAdmin="true" @close="showEditModal = false" @saved="refreshProfile" />
  </div>
</template>
